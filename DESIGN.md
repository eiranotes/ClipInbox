...
Contrast note: Primary and secondary text pairs pass normal-text contrast in both themes. Dark mode uses warm near-black surfaces and warm off-white text rather than pure black or pure white. Yellow keeps near-black text in both themes. When Increase Contrast is enabled, native `color.bg.cardMuted`, `color.text.secondary`, `color.text.tertiary`, and `color.border.soft` adapt to `#FFFFFF`/`#201E1A`, `#3F4247`/`#D8D3C9`, `#61666C`/`#B1ACA2`, and `#9F978A`/`#777168` respectively.
...
The native app implements the multi-line line heights with additive `lineSpacing` tokens: +3pt on multi-line titles, +6pt on body text and text editors, +4pt on multi-line meta text. Single-line labels stay at the font's natural height. Every native Pretendard token uses a semantic `relativeTo` text style so it scales with Dynamic Type; the Share Extension applies the matching `UIFontMetrics` style and enables automatic content-size adjustment.
...
Manual Add: the form remains scrollable, while the yellow save action is pinned in a bottom safe-area inset above the root navigation. It never scrolls behind the tab bar or appears only as a clipped sliver.

Search state: an empty query shows a short recent-clips preview labeled `최근 클립`; `검색 결과` is reserved for a real query. Recent-query suggestions disappear as soon as the user begins searching, and the empty copy explains whether the library or the current query has no result.
...
Badge and filter: type/state metadata is plain text with an optional semantic mark and appears only in focused/detail contexts. Inbox and search filters show five equal-width cells per row across two visible rows, with an 8px gap, a 44px touch target, and a 2px yellow active underline. The standard layout prefixes the rows with compact `폴더` and `태그` labels so the two scopes are unambiguous. In the Inbox, the top folder row and bottom tag row own separate horizontal scroll positions; other selectors keep the shared 5x2 grid. The first ten options fill the visible grid; additional options continue horizontally. Label length never changes cell width. At accessibility content sizes, the two dense rows become two native menu rows that summarize the current folder and tag selection without consuming the whole viewport. There is no per-filter box or duplicate filter modal.
...
Bottom navigation: white/card-muted surface, top separator, five stable tabs, yellow selected icon fill. Selected icon glyphs use `color.text.onAccent`; selected labels use `color.text.primary` so they remain visible in dark mode. Labels remain on one line and may scale down within their stable tab width at accessibility content sizes.
...
Detail image: the full source image uses aspect-fit inside a maximum 140px preview, even when that leaves horizontal or vertical breathing room. Tapping it opens a dark-compatible full-screen viewer with bounded pinch zoom, one-to-one panning, momentum, bounce, and double-tap zoom centered on the touched point.
...
Motion is light and functional: `--motion-fast` 140ms, `--motion-base` 180ms, `--motion-ease` cubic-bezier(0.2, 0.8, 0.2, 1). The keyboard is raised only by a direct tap on a text field or editor; screens and workflow sheets never request focus programmatically and do not synthesize a launch-time first-responder cycle. Tapping outside an input dismisses the keyboard on every input surface except the intentionally unchanged tag-selection sheet; the bottom navigation hides while the keyboard is visible instead of being lifted above it. Search result evaluation follows input by 120ms so Korean composition and key events stay responsive while results update after the user pauses. Pushed detail screens support a left-edge swipe that follows the finger one-to-one and springs back when canceled. Buttons and rows combine immediate opacity with restrained scale feedback; sheets use the system transition. Reduce Motion removes custom displacement and scale, avoids animated image zoom, and keeps state feedback instantaneous or opacity-only across root notices, metadata expansion, and Share Extension success.
